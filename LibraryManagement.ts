interface Book {
    BookID: number;
    Title: string;
    Author: string;
    Category: string;
    Available: boolean;
}
const books: Book[] = [
    { BookID: 1, Title: "The Great Gatsby", Author: "F. Scott Fitzgerald", Category: "Fiction", Available: true },
    { BookID: 2, Title: "Sapiens", Author: "Yuval Noah Harari", Category: "Non-Fiction", Available: false },
    { BookID: 3, Title: "1984", Author: "George Orwell", Category: "Fiction", Available: true },
    { BookID: 4, Title: "The Art of War", Author: "Sun Tzu", Category: "Philosophy", Available: true },
    { BookID: 5, Title: "Python Programming", Author: "John Doe", Category: "Technology", Available: false }
];
function displayAllBooks(): void {
    console.log("========================================================================================");
    console.log("ALL BOOKS");
    console.log("========================================================================================");
    console.log("ID\tTitle\t\t\t\tAuthor\t\t\tCategory\tAvailable");
    console.log("----------------------------------------------------------------------------------------");
    books.forEach(book => {
        console.log(`${book.BookID}\t${book.Title}\t\t${book.Author}\t\t${book.Category}\t\t${book.Available}`);
    });
    console.log("========================================================================================");
}
function displayAvailableBooks(): void {
    const availableBooks = books.filter(book => book.Available === true);
    console.log("\nAVAILABLE BOOKS");
    console.log("================================================================================");
    console.log("ID\tTitle\t\t\t\tAuthor\t\t\tCategory");
    console.log("--------------------------------------------------------------------------------");
    availableBooks.forEach(book => {
        console.log(`${book.BookID}\t${book.Title}\t\t${book.Author}\t\t${book.Category}`);
    });
    console.log("================================================================================");
}
function countAvailableBooks(): void {
    const availableCount = books.filter(book => book.Available === true).length;
    console.log(`\nTotal Available Books: ${availableCount} out of ${books.length}`);
    console.log("========================================");
}
displayAllBooks();
displayAvailableBooks();
countAvailableBooks();